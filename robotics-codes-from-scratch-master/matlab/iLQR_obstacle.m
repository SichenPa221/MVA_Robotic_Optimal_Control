%% iLQR applied to a 2D point-mass system reaching a target 
%% while avoiding obstacles defined by ellipses (avoidance cost 
%% in the form (1-e^2)'*Q*(1-e^2) = f'*f with f=U'*(1-e^2) and Q=U*U', 
%% which is a correct Gauss-Newton assumption)
%% 
...
for n=1:param.nbIter
	...
	du = (Su' * J' * J * Su * param.q + Su2' * J2' * J2 * Su2 * param.q2 + R) \ ...
	     (-Su' * J' * f(:) * param.q - Su2' * J2' * f2(:) * param.q2 - u * param.r); %Gauss-Newton update
...
plot(x(1,1:5:end), x(2,1:5:end), '.','markersize',15,'color',[0 0 0]);
