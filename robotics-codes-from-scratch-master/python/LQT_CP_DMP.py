...
x = np.load(str(Path(FILE_PATH, DATASET_LETTER_FILE)))[0,:,:param.nbVarU].T
...
xn = np.vstack((np.array([[3], [-0.5]]), np.zeros((param.nbVarX-2, 1))))  # Simulated noise on state
...
for t in range(param.nbData-2):
    id = np.arange((t+1) * param.nbVarU, (t+2) * param.nbVarU, step=1, dtype=int)
